...
import { purchaseData, customers, getTopProducts, getTopStyles } from '../data/mockData';
...
export function PurchaseAnalysis() {
  ...
  const wishlist   = purchaseData.filter(p => p.status === '찜');
  const cart       = purchaseData.filter(p => p.status === '장바구니');
  const viewOnly   = purchaseData.filter(p => p.status === '조회만함');
  ...
  const statusData = [
    { name:'구매완료', value: purchased.length, color:'#10b981' },
    { name:'찜',      value: wishlist.length,   color:'#f59e0b' },
    { name:'장바구니', value: cart.length,       color:'#3b82f6' },
    { name:'조회만함', value: viewOnly.length,   color:'#6b7280' },
  ];

  const catRev: Record<string, number> = {};
  purchased.forEach(p => { catRev[p.category] = (catRev[p.category]||0) + p.price; });
  const categoryData = Object.entries(catRev).map(([c,v]) => ({ category: c, revenue: v })).sort((a,b) => b.revenue - a.revenue);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label:'구매완료', value:`${purchased.length}건`, sub:`전환율 ${((purchased.length/purchaseData.length)*100).toFixed(1)}%`, icon: ShoppingBag, color:'bg-green-500' },
          { label:'찜 목록',  value:`${wishlist.length}건`,  sub:`비율 ${((wishlist.length/purchaseData.length)*100).toFixed(1)}%`,   icon: Heart,       color:'bg-pink-500' },
          { label:'장바구니', value:`${cart.length}건`,      sub:`비율 ${((cart.length/purchaseData.length)*100).toFixed(1)}%`,       icon: ShoppingCart, color:'bg-blue-500' },
          { label:'조회만함', value:`${viewOnly.length}건`,  sub:'이탈 개선 필요',                                                    icon: Eye,         color:'bg-gray-500' },
        ].map(m => {
          const Icon = m.icon;
          return (
          ...
          {topProducts.map((p, i) => (
            <div key={p.name} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">{p.category} · {p.style}</p>
              </div>
          ...
          {topStyles.map((s, i) => {
            ...
            return (
              <div key={s.style} className="flex items-center gap-3">
                <span className={`w-7 h-7 flex items-center justify-center rounded-full text-xs font-bold
                  ${i === 0 ? 'bg-yellow-400 text-white' : i === 1 ? 'bg-gray-400 text-white' : i === 2 ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-600'}`}>{i+1}</span>
                <span className="w-16 text-sm font-medium text-gray-900">{s.style}</span>
